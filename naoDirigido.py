...
def printarOpcoes(peso):
    ...
    if peso == False and op == 1:
        ...
        lista = criadorGrafo.lista_adjacencia_nao_dirigido_sem_peso(lista_vertices, arestas)
        print("\n\n\nLista de adjacência: \n")
        ...
        for vertice in lista_vertices:
            for indice, sub_vertice in enumerate(sub_lista):
                # Evita repetições já que o grafo não é dirigido
                if adjacentes.count(str(sub_vertice)+str(vertice))  == 0:
        ...
        lista = criadorGrafo.lista_adjacencia_nao_dirigido_com_peso(lista_vertices, arestas)
        print("\n\n\nLista de adjacência: \n")
        for  row in lista.items():
            print(row, end="\n")
